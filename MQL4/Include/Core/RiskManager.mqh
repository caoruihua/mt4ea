...
public:
   ...
   bool CheckCircuitBreaker(StrategyContext &ctx, RuntimeState &state, double dailyProfitStopUsd, double dailyLossStopUsd = 0)
   {
      ...
      // 达到日亏损阈值即锁定：当天禁止新开仓
      if(dailyLossStopUsd > 0 && state.dailyClosedProfit <= -dailyLossStopUsd)
         state.dailyLocked = true;
